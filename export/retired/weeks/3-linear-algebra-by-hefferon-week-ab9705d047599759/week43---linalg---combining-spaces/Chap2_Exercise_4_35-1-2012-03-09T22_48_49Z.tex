...
By induction, we can replace all k vectors to get $\mu^k$ and $\omega^k$, we can be sure that will keep finding non-trivial linear combinations for a vector $\vec{\beta_i}$ at step i, because both bases still span $\beta$ and  $\vec{\beta_i}$ is linearly indepent from any $\vec{\beta}$ vectors in the bases, so it must be dependent on the $\vec{\mu}$ and $\vec{\omega}$ remaining vectors.\\
